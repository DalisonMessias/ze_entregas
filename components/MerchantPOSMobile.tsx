...
import { useMerchantPOS, POSStep, PartialPayment } from '../hooks/useMerchantPOS';
...
export const MerchantPOSMobile: React.FC<MerchantPOSProps> = ({ onClose }) => {
    // Use custom hook for POS logic
    const {
        step, setStep,
    ...
    const settings = { enableHighContrast: false };
    const amountFontSize = useDynamicFont(amount, 60, 30, 1);

    // Specific Mobile Effects
    const [isDesktop, setIsDesktop] = useState(false);
    useEffect(() => { setIsDesktop(false); }, []);
    ...
    return (
        <div className={`fixed inset-0 bg-black/80 z-[200] flex items-center justify-center animate-in fade-in ${settings.enableHighContrast ? 'contrast-150 saturate-0' : ''}`}>
            ...
            <div className={`w-full h-full ${isDesktop ? 'max-w-5xl' : 'sm:max-w-sm'} bg-brand-600 shadow-2xl overflow-hidden relative flex flex-col p-2 sm:rounded-xl`}>
