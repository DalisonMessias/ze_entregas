...
export const AdminClaims: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            if (activeTab === 'claims') {
                ...
                setShopSettings(settings || { id: true });
            }
        } catch (e) {
    ...
    const handleUpdateClaim = async (status: 'open' | 'resolved' | 'closed') => {
        ...
        setProcessing(true);
        try {
            await cloud.adminUpdateClaim(selectedClaim.id, adminResponse, status);
            await loadData();
            setSelectedClaim(null);
            setAdminResponse('');
        } catch (e: any) {
            console.error(e);
            await alert({ title: 'Chamado', message: 'Erro ao atualizar chamado: ' + e.message });
        } finally {
            setProcessing(false);
        }
    };
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            ...
            await alert({ title: 'Suporte', message: 'Configurações de suporte salvas!' });
        } catch (e: any) {
