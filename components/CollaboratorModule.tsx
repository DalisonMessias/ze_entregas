...
type View = 'dashboard' | 'menu' | 'tables' | 'history' | 'reports' | 'external_order';
...
export const CollaboratorModule: React.FC<Props> = ({ collaborator, onLogout }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col font-sans select-none">
                                ...
                                {openOrders.map(order => (
                                    <div key={order.id} className="bg-white dark:bg-gray-800 p-6 rounded-[32px] shadow-sm border border-gray-50 dark:border-gray-700 group hover:shadow-xl transition-all">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-[10px] text-gray-400 font-bold">
                                                        ...
                                                        <span className="ml-2 text-brand-500/50">
                                                            ({Math.floor((new Date().getTime() - new Date(order.updated_at).getTime()) / 60000)} min)
                                                        </span>
                                                    </span>
                                                </div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Button fullWidth onClick={() => setShowPreCheck(order)} className="rounded-2xl text-xs py-3 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">Conferir</Button>
            ...
            {/* FAB (Botão Flutuante) apenas no dashboard */}
            {
                view === 'dashboard' && (
                    <button
                        onClick={() => { setView('menu'); setSelectedOrderId(null); setTableName(''); setCustomerName(''); }}
                        className="fixed bottom-8 right-8 w-16 h-16 bg-brand-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-40"
                    >
