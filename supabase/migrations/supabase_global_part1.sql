...
CREATE TABLE IF NOT EXISTS public.partner_fee_settings (
    ...
    super_store_commission_enabled BOOLEAN DEFAULT TRUE,
...
UPDATE public.partner_fee_settings SET super_store_commission_enabled = TRUE WHERE id = '1' AND super_store_commission_enabled IS FALSE;
...
-- Função: get_store_dashboard_stats (Dados completos do lojista)
...
DECLARE
  ...
  v_failed INT := 0;
  v_sales_days INT := 7;
  v_since TIMESTAMPTZ := now() - (v_sales_days || ' days')::interval;
BEGIN
  -- Totais de pedidos
  SELECT COALESCE(COUNT(*), 0) INTO v_total_requests FROM (
    SELECT id FROM public.orders WHERE store_id = v_store
    UNION ALL
    SELECT id FROM public.orders_collaborators WHERE store_id = v_store
  ) s;

  -- Faturamento total
  SELECT COALESCE(SUM(o.total_price), 0) INTO v_total_value
  FROM public.orders o
  WHERE o.store_id = v_store;

  SELECT v_total_value + COALESCE(SUM(oc.total_amount), 0) INTO v_total_value
  FROM public.orders_collaborators oc
  WHERE oc.store_id = v_store;

  -- Contagens por status (app)
  SELECT
    COALESCE(SUM(CASE WHEN UPPER(o.status::text) IN ('DELIVERED', 'COMPLETED') THEN 1 ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN UPPER(o.status::text) IN ('CANCELLED') THEN 1 ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN UPPER(o.status::text) IN ('FAILED', 'EXPIRED') THEN 1 ELSE 0 END), 0)
  INTO v_completed, v_cancelled, v_failed
  FROM public.orders o
  WHERE o.store_id = v_store;

  -- Contagens por status (mesas/colaboradores)
  SELECT
    v_completed + COALESCE(SUM(CASE WHEN UPPER(oc.status::text) IN ('COMPLETED') THEN 1 ELSE 0 END), 0),
    v_cancelled + COALESCE(SUM(CASE WHEN UPPER(oc.status::text) IN ('CANCELLED') THEN 1 ELSE 0 END), 0),
    v_failed + COALESCE(SUM(CASE WHEN UPPER(oc.status::text) IN ('FAILED', 'EXPIRED') THEN 1 ELSE 0 END), 0)
  INTO v_completed, v_cancelled, v_failed
  FROM public.orders_collaborators oc
  WHERE oc.store_id = v_store;
  ...
  RETURN jsonb_build_object(
    ...
    'peakHours', COALESCE((
      WITH hours AS (
        SELECT EXTRACT(HOUR FROM o.created_at)::int AS hour, COUNT(*) AS count
        FROM public.orders o
        WHERE o.store_id = v_store
        GROUP BY 1
        UNION ALL
        SELECT EXTRACT(HOUR FROM oc.created_at)::int AS hour, COUNT(*) AS count
        FROM public.orders_collaborators oc
        WHERE oc.store_id = v_store
        GROUP BY 1
      )
      SELECT jsonb_agg(jsonb_build_object('hour', hour, 'count', total_count) ORDER BY hour)
      FROM (
    ...
    'driverPerformance', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'count', pr.total_count
      ) ORDER BY pr.total_count DESC)
      FROM (
        SELECT
          pr.partner_id,
          COALESCE((SELECT name FROM public.user_profiles WHERE id = pr.partner_id), 'Entregador') AS partner_name,
          COUNT(*) AS total_count
        FROM public.partner_requests pr
        WHERE pr.store_id = v_store AND pr.partner_id IS NOT NULL
        GROUP BY pr.partner_id
        ORDER BY total_count DESC
      ) pr
    ), '[]'::jsonb),
    'lowSalesProducts', COALESCE((
      WITH items_app AS (
        SELECT
          CASE
        ...
        WHERE o.store_id = v_store AND o.created_at >= v_since
      ),
      items_table AS (
        SELECT
          oi.product_id,
          COALESCE(oi.name, sp.name) AS name,
          COALESCE(oi.quantity, 0)::numeric AS quantity,
          COALESCE(oi.total_price, oi.unit_price * oi.quantity, 0) AS revenue
        FROM public.orders_items oi
        JOIN public.orders_collaborators oc ON oc.id = oi.order_id
        LEFT JOIN public.store_products sp ON sp.id = oi.product_id
        WHERE oc.store_id = v_store AND oc.created_at >= v_since
      ),
      combined_sales AS (
        SELECT
          s.product_id,
      ...
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'revenue', ls.revenue
      ) ORDER BY ls.quantity ASC, ls.name ASC)
      FROM (
        SELECT
          sp.id AS product_id,
          COALESCE(sp.name, cs.name, 'Produto') AS name,
          COALESCE(cs.quantity, 0) AS quantity,
          COALESCE(cs.revenue, 0) AS revenue
        FROM public.store_products sp
        LEFT JOIN combined_sales cs ON cs.product_id = sp.id
        WHERE sp.store_id = v_store
        ORDER BY COALESCE(cs.quantity, 0) ASC, sp.name ASC
        LIMIT 5
      ) ls
    ), '[]'::jsonb),
    'salesWindowDays', v_sales_days
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  SET is_super_store = TRUE, 
      ...
      super_store_expiration = now() + interval '30 days',
      updated_at = now() 
  WHERE id = v_user;
  ...
  IF p_plan_type = 'MENSALIDADE' AND fee > 0 THEN
      ...
      UPDATE public.store_wallets
      SET balance_decimal = balance_decimal - ABS(fee)
      WHERE store_id = v_user; -- Corrigido para usar store_id
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Tabela de Comissﾃｵes da Plataforma
